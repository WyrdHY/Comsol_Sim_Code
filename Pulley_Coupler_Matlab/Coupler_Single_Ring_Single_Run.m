...
mphstart(2036); 
%%
modelPath1 = "C:\Users\Dirk\Desktop\Hongrui_Yan_Simulation\Model\PG_Coupler_Jan_9_2025\Single Ring_Body.mph";
savePath1 = "C:\Users\Dirk\Desktop\Hongrui_Yan_Simulation\Model\PG_Coupler_Jan_9_2025\Single Ring_Body.mph";

modelPath2 = "C:\Users\Dirk\Desktop\Hongrui_Yan_Simulation\Model\PG_Coupler_Jan_9_2025\Single Ring_Bus.mph";
savePath2 = "C:\Users\Dirk\Desktop\Hongrui_Yan_Simulation\Model\PG_Coupler_Jan_9_2025\Single Ring_Bus.mph";
...
model1.result('pg1').feature('arws1').set('expr', {'abs(ewfd2.Er)','abs(ewfd2.Ez)'});
mphplot(model1, 'pg1');
drawnow;
%% Bus
if ida > 800
    ...
    width_bus_range = [5.91,5.92,5.93,5.94,5.95]; 
...
for width_bus = width_bus_range
    ...
    model2.param.set('width',[num2str(width_bus,'%.5f'),'[um]'] );
    ...
    for gap_distance = gap_range
        disp(['     Running gap_distance = ', num2str(gap_distance), ' um ',num2str(jj/all*100),'%']);
        
        % Update Parameters for Current Gap Distance
        radius2 = radius + gap_distance + width_bus / 2 + width_body / 2;
        sim_width = 1.5;
        initial_guess = radius2*nCore*1e-6;

        % First Run: Eigenvalue Calculation
        model2.param.set('nCore', num2str(nCore, '%.7f'));
        model2.param.set('radius', [num2str(radius2, '%.4f'), '[um]']);
        model2.param.set('initial_guess', 'nCore*radius');
        model2.param.set('interest_width', [num2str(sim_width, '%.4f'), '[um]']);
        ...
        sim_width = width_bus*2;
        model2.param.set('initial_guess', num2str(initial_guess, '%.6f'));
        model2.param.set('interest_width', [num2str(sim_width, '%.4f'), '[um]']);
        model2.study(studyName).run();

        % Extract TE and TM Values
        [~, iTE] = max(mphglobal(model2, 'ex2', 'dataset', indicator.dset));
        [~, iTM] = max(mphglobal(model2, 'ez2', 'dataset', indicator.dset));
        check = mphglobal(model2, 'core_energy2/total_energy2', 'dataset', indicator.dset);
        %figure;
        %model2.result('pg1').set('data', indicator.dset);
        %model2.result('pg1').set('solnum', num2str(iTE));
        %model2.result('pg1').feature('surf1').set('expr', indicator.normE); % Ensure 'surf1' exists in pg1
        %model2.result('pg1').feature('surf1').set('descr', '|E|');  
        %model2.result('pg1').feature('arws1').set('expr', {'abs(ewfd2.Er)','abs(ewfd2.Ez)'});
        %mphplot(model2, 'pg1');
        %drawnow;
        if check(iTE) > 0.7
...
% Base folder path
base_folder_path = 'C:\Users\Dirk\Desktop\Hongrui_Yan_Simulation\Model\PG_Coupler_Jan_9_2025\log2';
